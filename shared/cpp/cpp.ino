...
struct Sensor
{
  ...
  bool isHealthy = true;   // true if sensor is working, false if failed
  bool prevHealthy = true; // previous health state for edge detection
  int lineNumber = -1;     // which line this sensor corresponds to (for QueueManager updates)
};
...
void updateSensor(Sensor &S)
{
  ...
  S.prevHealthy = S.isHealthy;
  S.isHealthy = !isnan(cm);

  // Detect sensor health changes and update QueueManager
  if (S.prevHealthy != S.isHealthy && S.lineNumber > 0 && g_qm)
  {
    if (S.isHealthy)
    {
      g_qm->setLineAvailability(S.lineNumber, true);
    }
    else
    {
      g_qm->setLineAvailability(S.lineNumber, false);
    }
  }
...
void setup()
{
  ...
  // Associate sensors with their corresponding lines for health tracking
  // Sensor A (hostess) doesn't control a specific line - it's for entry
  A.lineNumber = -1;                // Entry sensor, doesn't control line availability
  B.lineNumber = EXIT1_LINE_NUMBER; // Sensor B controls line 1 availability
  C.lineNumber = EXIT2_LINE_NUMBER; // Sensor C controls line 2 availability
